...
/* ---------- Persistency (localStorage por slug) ---------- */
type ShowStore = {
...
function ShowPanel({ show, onClose }: { show: Show; onClose: () => void }) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <aside
        className="fixed right-0 top-0 z-50 h-full w-[min(40vw,640px)] overflow-y-auto border-l border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950"
      ...
      >
        ...
        <div className="sticky top-0 z-10 -mx-5 mb-4 flex items-center justify-between border-b border-zinc-200 bg-white/80 px-5 py-3 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/70">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="rounded-lg border border-zinc-300 px-2 py-1 text-xs hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800">
              Edit
            </button>
            <button className="rounded-lg border border-zinc-300 px-2 py-1 text-xs hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800">
              View full page
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section title="Transport">
            <div className="flex flex-wrap gap-2 text-xs">
              ...
              <TransportPill value="van" label="Van" icon={<Hotel className="hidden" />} />
            </div>
          </Section>
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (slug) {
      const found = SHOWS.find(s => s.slug === slug);
  ...
  const openShow = (s: Show) => {
    const params = new URLSearchParams(Array.from(search?.entries() ?? []));
    params.set("show", s.slug);
  ...
  return (
    ...
    <div>
                  ...
                  back={
                    <div className="flex h-full w-full flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); openShow(s); }}
                          className="rounded-lg border border-zinc-300 px-2 py-1 text-xs hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800"
                        >
                          Open
                        </button>
      ...
      {/* Slide-over */}
      {active && <ShowPanel show={active} onClose={closeShow} />}
